---
type: research
project: renuvex-product-reviews
status: draft
created: 2026-05-05
updated: 2026-05-05
tags:
  - research
  - market
related:
  - "[[Index]]"
  - "[[Competitor_Insights]]"
---

# Review App Market Research

## Summary
Placeholder for market sizing, segment analysis, pricing benchmarks, and acquisition channels. Fill as research is done.

## Open questions
- Pricing tiers used by Loox / Judge.me / Yotpo / Okendo (TRY conversions)
- Avg # of reviews per merchant after 6 months on platform — benchmark
- Conversion lift from displaying reviews (industry studies)
- TR market specifics — language, payment, support expectations

## To do
- Compile pricing matrix
- Talk to 3 ikas merchants currently using ad-hoc / external review apps
- Survey TR-specific review patterns (KVKK / data residency)

## Obsidian Links
- [[Competitor_Feature_Matrix]]
- [[Competitor_Insights]]
- [[Roadmap]]
