...
as `.renuvex-pr-star` / `.renuvex-pr-stars-partial` spans wrapping inline SVGs. Each `.renuvex-pr-star`
...
`#renuvex-pr-styles` stylesheet.

`#renuvex-pr-styles` is injected by `injectStyles()` ([helpers.js](src/widget/core/helpers.js)),
...
Result: cold listing entry (no PDP rendered) → `#renuvex-pr-styles` absent → `.renuvex-pr-star`
...
which injects `#renuvex-pr-styles` into `<head>`; it then persists for the SPA session,
so returning to a listing page shows stars.

Confirmed live: cold home / `/shorts` / `/search` / `/clothing` all showed
`.renuvex-pr-star` computed `display:inline` and `getBoundingClientRect()` `0×0`.
...
- [helpers.js](src/widget/core/helpers.js) — the partial-stars CSS block
  (`.renuvex-pr-stars-partial` / `.renuvex-pr-star*`) is extracted into a single exported
...
- [themes/ozy/styles.js](src/widget/themes/ozy/styles.js) — `CLASSIC_CSS`
  ...
  `#renuvex-pr-styles` output is unchanged.
...
`PARTIAL_STARS_CSS` constant, the two copies cannot drift. `#renuvex-pr-badge-styles`
and `#renuvex-pr-styles` carry identical `.renuvex-pr-star` rules and coexist harmlessly on a PDP.
Requires `pnpm build:widget` to land in `public/widget.js`.

## Verification (2026-05-17, dev store, post-fix build)
- Cold home entry (no PDP visited): `#renuvex-pr-badge-styles` present, `#renuvex-pr-styles`
  absent, `.renuvex-pr-star` `display:flex`, size `13×13`, half-star `clip-path` applied.
- Cold category `/shorts` (no PDP visited): same — stars render `13×13`.
- PDP: `#renuvex-pr-styles` still injected with star rules; rating badge stars `16×16`;
  JSON-LD / `.renuvex-pr-summary` / `.renuvex-pr-title` each count 1 (no double render); no
