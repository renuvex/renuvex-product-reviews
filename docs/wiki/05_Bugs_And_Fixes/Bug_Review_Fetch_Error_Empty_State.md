...
## Root Cause
- [bootstrap.js](src/widget/reviews-section/bootstrap.js) returned `null` from `fetchReviews()` for fetch failures when no stale cache existed.
- [render.js](src/widget/reviews-section/render.js) normalized `null` to `{}` and then derived `reviews = []`, which is the same branch used for a real empty review list.
- Load-more treated malformed/null review data as terminal and removed the button.

## Fix
- [bootstrap.js](src/widget/reviews-section/bootstrap.js) now returns an explicit review fetch error result when fresh and stale review data are both unavailable.
- [render.js](src/widget/reviews-section/render.js) detects the error result and renders a retryable error state instead of the empty review state.
