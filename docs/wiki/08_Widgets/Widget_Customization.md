...
- `Video Oynatıcı` is a storefront review-lightbox-only color group. It exposes five explicit colors: play icon, play button background, button hover background, progress color, and progress track color. The saved keys use the `reviewLightboxVideo*` prefix so the admin moderation player and future story/carousel video players remain isolated instead of inheriting this surface accidentally.
...
## Change Log
- 2026-06-24: Added the `Video Oynatıcı` color group for the product review lightbox Mux Player. The schema adds five `reviewLightboxVideo*` keys and the runtime maps them through `theme-vars.js` into the storefront Mux Player theme. Admin moderation playback and future video surfaces are intentionally not coupled to these keys.
