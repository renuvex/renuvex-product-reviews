...
- 2026-05-07: Modified the `boxed` (Box Square) icon's `empty` state architecture. Previously, it used an outline variant for the box and the inner star, causing visual noise due to double-stroking. It now uses the `filled` variant with an inline `opacity="0.35"` attribute, matching the industry standard (e.g., Loox) for complex container-based icons. Normal star and heart icons retain the 100% opaque outline architecture as they are simple, single-stroke shapes. Related source: [icons.js](src/widget/icons.js).
- 2026-05-07: Implemented a stroke-based outline for the `favorite` (Heart) icon to fix a visual disjoint in the half-state. The official Google `heartOutline` path has different vertical alignment/height compared to the `heartRounded` path, causing a "step" or "break" at the bottom tip when combined in a 4.5 rating. The empty state now uses the exact same path as the filled state (`P.heartRounded`) with `fill="none"` and `stroke-width="72"`, ensuring perfect geometric synchronization at the 50% clip line. Related source: [icons.js](src/widget/icons.js).
