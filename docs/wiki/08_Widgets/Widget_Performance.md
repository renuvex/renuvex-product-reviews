...
- 2026-07-03 AWS image teardown: widget upload/register no longer emits legacy upload fields or trusts legacy render URLs. New image media uses AWS `variantManifest` descriptors and public URLs under `https://media.renuvex.app/reviews/<assetId>/<variant>.<format>`; public reads stay DB-only.
