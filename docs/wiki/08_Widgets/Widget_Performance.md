...
- 2026-06-25 media filter count path: video-enabled stores now use `hasMedia=true` for the shopper-facing `Fotoğraf ve Video` filter. `ProductReviewSummary.mediaReviewCount` / `mediaRating*Count` keep those totals read-model backed instead of raw `Review.count()`.
