...
## Change Log
- 2026-06-11: Product-empty review UI (`allCount === 0`) moved into `buildEmptyReviewsState()` in [render/states.js](src/widget/reviews-section/render/states.js). It preserves the existing centered write-review CTA + "Henüz yorum yok." copy, adds `role="status"` / `aria-live="polite"` to the message, and widget-runtime smoke now pins the empty-state contract including custom `writeButtonText`.
