...
2. Fetches `/api/public/settings` (cached), then calls `reviews-api.js` for `/api/public/reviews` and the media-gallery fetch (cached; image-only stores use `hasImages=true`, video-enabled stores use `hasMedia=true`; stale reviews are preferred on failures).
...
- Filter: by rating (1..5), and by a media facet. Video-disabled stores show the legacy `Fotoğraflı` filter and send `hasImages=true`. Video-enabled stores show `Fotoğraf ve Video` and send `hasMedia=true` (`Review.hasImages OR Review.hasVideo`). The widget tracks this as an explicit `currentMediaFilter` mode (`none | images | media`) rather than a photo-only boolean. The rating filter is driven by the summary bar chart ([summary-layouts/shared/bar-chart.js](src/widget/summary-layouts/shared/bar-chart.js)); a bar with **zero reviews is not interactive** (no `role="button"`, not focusable, no hover, `cursor:default`), so a click can't land on an empty "Henüz yorum yok" result — only bars that actually have reviews filter (Looox-style).
...
- Image-only stores keep the existing `hasImages=true&limit=15&orderBy=newest` fetch. Video-enabled stores use `hasMedia=true&limit=15&orderBy=newest` so approved video posters can appear in the same gallery. The top gallery remains bootstrap-owned and hides when the review list is already filtered by the photo/media facet.
...
## Change Log
- 2026-06-25: The summary filter media facet became adaptive. Video-disabled stores keep `Fotoğraflı` / `hasImages=true`; video-enabled stores show `Fotoğraf ve Video` / `hasMedia=true`. `ProductReviewSummary` now owns media-filter count buckets so public media filtering stays on the read-model path instead of raw `Review.count()`.
