...
- `Widget Boyutu` (`size`) is also the single source for list-pagination control sizing. Desktop load-more and numbered pagination use compact visible boxes. Mobile/coarse-pointer layouts split the control into a visible box and a minimum tap target: the visible box scales by size, while the focusable button keeps a 44px hit target. No separate pagination-size admin setting exists, and centered wrap behavior stays unchanged.
...
## Change Log
- 2026-06-12: Review pagination sizing was revised again: desktop load-more/numbered controls are more compact, while mobile uses an inner visible label box plus a 44px button hit target so `small` / `medium` / `large` remain visually distinct without shrinking the tap area. Runtime smoke covers desktop visible size, mobile visible size, hit target, focus ring placement, and overflow.
