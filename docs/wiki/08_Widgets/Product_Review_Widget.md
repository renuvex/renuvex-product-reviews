...
## Change Log
- 2026-06-22: The video-enabled submission wizard media step now uses two stacked primary actions instead of the old side-by-side outline choices. The embedded photo picker hides its internal add button, preventing the duplicate `Fotoğraf Ekle` control while preserving the existing photo upload lifecycle. Empty embedded media content stays hidden until a real photo/video selection exists, so picker clicks do not move the action buttons. The video action uses the Phosphor regular outline play icon via the shared icon registry.
