...
## Change Log
- 2026-06-09: Pagination active page gained its own `paginationActiveBgColor` setting (default `#111111`), decoupled from `paginationTextColor`; the active number color now auto-derives for contrast (`getReadableControlColor`). Previously the active fill was the inverted number color, so changing "Numara Rengi" also moved the active box. Default look is unchanged.
