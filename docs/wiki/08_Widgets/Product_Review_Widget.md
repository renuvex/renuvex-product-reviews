...
2. Fetches `/api/public/settings` (cached), then calls `reviews-api.js` for `/api/public/reviews` and the media-gallery fetch (cached; the gallery uses `hasMedia=true` independently from new video-upload capability; stale reviews are preferred on failures).
...
- Photos upload through `/api/public/upload/sign` and `/api/public/upload/register`: the widget receives an AWS S3 presigned POST, uploads directly to S3, then registers the object for server-side validation and variant generation.
- Video V1 is opt-in and gated by global flag + merchant setting + provider configuration + the current UTC month's `reservedCount + consumedCount < monthlyLimit`. The cached settings response expresses merchant intent; the fresh capability endpoint expresses current upload availability. When cached settings enable video, step 2 starts as a media-capable step while capability is pending: `Fotoğraf Ekle` is active and `Video Ekle` is visible but disabled/`aria-busy`; clicking the pending video action cannot open the file picker, start a Mux session, or affect quota. When capability resolves enabled, `Video Ekle` becomes active. When capability resolves unavailable, the wizard falls back to the photo-only step. Admin-disabled video skips the capability probe entirely and stays photo-only. When effective, step 2 becomes a media step ([step-media.js](src/widget/reviews-section/review-form-modal/steps/step-media.js)): users can add **3 photos OR 1 video**, never mixed media in v1. The photo path reuses [step-photos.js](src/widget/reviews-section/review-form-modal/steps/step-photos.js) in embedded mode so thumbnails and upload lifecycle stay shared. The embedded picker keeps its own add control hidden before media exists; after a photo is pending or uploaded, the outer media actions hide and the embedded photo thumbnail rail shows the selected thumbnail(s) plus the compact `+` tile. Empty embedded media content stays hidden until actual media exists, so opening the native photo or video picker without selecting media does not shift the action buttons. Removing a selected video resets step 2 to the empty media-selection state.
...
- On submit → `POST /api/public/reviews`; AWS image refs are validated against store/product/session ownership before storage. A ready video token is validated against store/product/session ownership, consumes the pending video, forces `Review.status='pending'`, and waits for admin moderation before public visibility.
- The legacy inline/page form was removed; all review CTAs open the multi-step modal.

## Pagination, filtering, sorting
- Pagination: 10 per page (server-side); `limit` query param clamped 1-30 for ad-hoc fetches (media gallery uses 15).
- Sort: `newest` / `highest` / `lowest`.
- Filter: by rating (1..5), and by a media facet. New video-upload availability does **not** decide the public media filter. The public reviews response exposes `photoReviewCount` and `mediaReviewCount`; when `mediaReviewCount > photoReviewCount`, the widget shows `Fotoğraf ve Video` and sends `hasMedia=true` (`Review.hasImages OR Review.hasVideo`) so existing approved videos stay visible even if new uploads are disabled. Otherwise it keeps `Fotoğraflı` and sends `hasImages=true`. The widget tracks this as an explicit `currentMediaFilter` mode (`none | images | media`) rather than a photo-only boolean. The rating filter is driven by the summary bar chart ([summary-layouts/shared/bar-chart.js](src/widget/summary-layouts/shared/bar-chart.js)); a bar with **zero reviews is not interactive** (no `role="button"`, not focusable, no hover, `cursor:default`), so a click can't land on an empty "Henüz yorum yok" result — only bars that actually have reviews filter (Looox-style).
- Bar chart in summary uses filter-independent `ratingCounts` returned by `/api/public/reviews`. Aggregate fields (`allCount`, `avgRating`, `ratingCounts`) and exact `totalCount` / `totalPages` for rating/photo/media filters come from the backend `ProductReviewSummary` read model; only the visible review rows come from `Review`. See [[ADR_0026_Product_Review_Summary_Read_Model]].
...
- 2026-06-08: Public review responses now include additive `media[]` metadata entries after signed upload-response verification in the previous image-provider path. `images: string[]` remains the widget compatibility contract. Related ADR: [[ADR_0029_Review_Media_Metadata]].
...
- 2026-05-31: Wizard/lightbox lifecycle audit found the then-photo-strip lightbox thumbnails were click-only images. Added shared `wireLightboxTrigger()` and moved card/list/gallery triggers to it; interaction smoke now verifies keyboard open and focus restore from the media gallery.
- 2026-05-27: ADR_0024 follow-up moved review/media-gallery fetch helpers into [reviews-api.js](src/widget/reviews-section/reviews-api.js). `bootstrap.js` is now review mount orchestration, while `render.js` uses the same fetch helper for retry/filter/sort/load-more interactions.
...
- 2026-05-11: The historical photo strip decoupled from main reviews fetch — dedicated `hasImages=true&limit=15&orderBy=newest` call, cap 15, newest-first rotation. The active surface is now [[Media_Gallery]], but the independence decision remains [[ADR_0007_Photo_Strip_Cap_And_Rotation]]. Source: [reviews-api.js](src/widget/reviews-section/reviews-api.js), [bootstrap.js](src/widget/reviews-section/bootstrap.js), [render.js](src/widget/reviews-section/render.js), [state.js](src/widget/core/state.js), [route.ts](src/app/api/public/reviews/route.ts).
