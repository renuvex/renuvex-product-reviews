...
- Video entry points use [core/review-media.js](src/widget/core/review-media.js): storefront renders only trusted Mux HLS/poster URLs from normalized `media[]`; provider ids never reach the widget. Cards/lists/gallery/strip show poster + play badge + optional duration, not autoplaying `<video>` elements.
...
- Videos upload through [media/video-upload.js](src/widget/reviews-section/review-form-modal/media/video-upload.js): client validates MP4/MOV and <=150MB immediately, then auto-advances to the comment step while duration validation, Mux direct upload, complete, and processing polling continue in the shared wizard state. Returning to step 2 during upload/retry/processing shows a single black `Video yükleniyor` status control without a remove button; when UpChunk or the browser reports offline, the same control switches to `İnternet bağlantısı yok` and returns to `Video yükleniyor` when connectivity resumes. Offline pauses keep a low-frequency network probe alive so missed or reordered browser `online` events cannot leave the direct-upload watchdog permanently paused. Once online, the upload path requires real activity evidence from UpChunk (`attempt`, `attemptFailure`, `chunkSuccess`, `progress`, `success`, or terminal error) or server status. If no activity arrives, the client aborts the stalled direct-upload request once, reads the current session status, and retries the same selected file against the still-valid upload URL instead of waiting indefinitely. Automatic same-session recovery is limited to watchdog-detected stalls; exhausted UpChunk chunk attempts still surface `Video yüklenemedi` + `Tekrar dene`. Once ready, step 2 shows one photo-sized video thumbnail with only the remove button, no play icon and no extra `+` video tile. The app stores only the opaque session token in widget state; Mux upload URL is not exposed in public API responses. Server-side Mux asset/webhook/reconcile state remains authoritative; client validation is only fast feedback.
- Upload failures keep detailed server error codes in state/metrics, but the storefront card uses generic shopper copy: `Video yüklenemedi`. Retryable failures show one themed `Tekrar dene` action that preserves the selected file and existing upload session when status says the session is still valid; only expired, failed, aborted, or missing sessions are discarded before a new session is created. Non-retryable failures show no retry action. Busy/offline states show no buttons or remove icon.
...
## Change Log
- 2026-06-22: Hardened the video direct-upload offline recovery after a physical mobile Wi-Fi disconnect/reconnect test showed the wizard could remain indefinitely on `Video yükleniyor`. The upload layer now listens to browser network events in addition to UpChunk events, keeps a network probe alive while offline so missed `online` events re-arm the stall watchdog, and keeps automatic recovery limited to watchdog-detected stalls. Interaction smoke now covers offline pause -> online re-arm -> resolution without changing the existing chunk-attempt-exhausted manual retry contract.
- 2026-06-22: The video-enabled submission wizard media step now auto-advances to the comment step after video selection while Mux upload/processing continues in shared state. Returning to step 2 shows a single black loading status during upload/retry/processing, a distinct `İnternet bağlantısı yok` state only when UpChunk reports offline, and a single photo-sized removable video thumbnail once ready. Ready thumbnails have no play icon and no extra `+` video tile. The same date also replaced the old side-by-side media choices with two stacked primary actions, hid empty embedded media content to prevent layout shift, aligned the photo-selected media state so primary actions hide while the embedded thumbnail strip exposes the compact `+` add tile, and added the Mux direct-upload activity watchdog/same-file retry path so online stalls cannot leave the wizard indefinitely on `Video yükleniyor`.
