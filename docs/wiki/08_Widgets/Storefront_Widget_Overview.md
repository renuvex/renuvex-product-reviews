...
V2 source supports an allowlisted read-through cache for `GET /api/public/settings`, `GET /api/public/ratings`, and `GET /api/public/reviews`. The DOM-only `GET /api/public/ratings-by-slug` route is still allowlisted as a pass-through, but product-lifecycle ambiguity makes both the backend and Worker response `no-store`. Exact product-id rating results may use the widget's short session cache; slug-only results do not. Every write/upload/video/widget-error/lazy-sync route stays on `app.renuvex.app`. The Worker remains fail-closed for non-allowlisted `/api/*` paths. See [[ADR_0037_Product_Lifecycle_Evidence_And_Tombstones]].
