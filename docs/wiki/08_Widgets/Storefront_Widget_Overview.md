...
Use this page for storefront widget surfaces, split API/static origins, and
Worker delivery. Treat source, deployed Vercel backend, and serving Cloudflare
Worker as separate evidence layers; current slug no-store rollout status is in
...
V2 uses an allowlisted read-through cache for `GET /api/public/settings`, `GET /api/public/ratings`, and `GET /api/public/reviews`. The DOM-only `GET /api/public/ratings-by-slug` route is allowlisted only as a pass-through: product-lifecycle ambiguity requires backend and Worker responses to remain `no-store`. Exact product-id rating results may use the widget's short session cache; slug-only results do not. The 2026-08-09 `A0-EDGE` acceptance verified repeated `no-store/DYNAMIC/BYPASS` immediately and after the old five-minute runtime window, with no edge `HIT`. Every write/upload/video/widget-error/lazy-sync route stays on `app.renuvex.app`. The Worker remains fail-closed for non-allowlisted `/api/*` paths. See [[ADR_0037_Product_Lifecycle_Evidence_And_Tombstones]].
...
## Change Log
- 2026-08-09: Worker version `0bc1674d-331e-4953-a192-7f72c32d0fc4`
  closed `A0-EDGE`; repeated immediate and five-minute slug probes returned
  `no-store/DYNAMIC/BYPASS` without edge hits.
- 2026-08-08: Distinguished the merged slug no-store source contract from the
  still-cacheable serving Worker and linked the explicit edge rollout gate.
