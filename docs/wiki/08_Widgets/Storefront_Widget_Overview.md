...
V2 source adds an allowlisted read-through cache for only `GET /api/public/ratings`, `GET /api/public/ratings-by-slug`, and `GET /api/public/reviews`. `GET /api/public/settings` and every write/upload/video route stay on `app.renuvex.app`. Live V2 activation requires a separate Worker deploy plus Vercel `STOREFRONT_WIDGET_READ_API_BASE_URL` cutover.
