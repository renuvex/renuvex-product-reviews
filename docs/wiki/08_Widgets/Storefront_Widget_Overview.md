...
V2 source supports an allowlisted read-through cache for `GET /api/public/settings`, `GET /api/public/ratings`, and `GET /api/public/reviews`. The DOM-only `GET /api/public/ratings-by-slug` route is still allowlisted as a pass-through, but product-lifecycle ambiguity makes the backend and current Worker **source** response `no-store`. Exact product-id rating results may use the widget's short session cache; slug-only source results do not. A 2026-08-08 live probe still observed `MISS` then `HIT` because the serving Worker predates the Release A source; do not call the no-store edge contract live until `A0-EDGE` acceptance in [[Product_Lifecycle_Scale_And_Retention_Audit_2026-08-03]] passes. Every write/upload/video/widget-error/lazy-sync route stays on `app.renuvex.app`. The Worker remains fail-closed for non-allowlisted `/api/*` paths. See [[ADR_0037_Product_Lifecycle_Evidence_And_Tombstones]].
