...
## In Progress
- Video review production acceptance: the internal-store provider chain is active for controlled physical-device acceptance, while the second store remains effectively disabled. Quota-aware capability commit `84276d8b` is deployed in Production as `dpl_AjWAf29bnLSQWxA8ceQ8gdLBQQiz`; the internal quota is `20`. Android passed the earlier resume flow and retained-review check, but the new media-path reliability hardening changes the acceptance baseline. Android and iPhone interruption/offline-cancel/processing tests must be repeated after deployment, then the 72-hour clock can start at a new explicit `T0`. See [[Review_Video_Canary_Runbook]], [[Review_Video_Physical_Device_Acceptance_2026-06]], and [[ADR_0031_Review_Media_V2_Provider_Agnostic_Video]].
