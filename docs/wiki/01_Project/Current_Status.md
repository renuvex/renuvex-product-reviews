...
## In Progress
- Video review production acceptance: the internal-store provider chain has produced two consumed quota units and completed prepare/publish/ingest-cleanup/video-cleanup jobs without retained media. Pending-admin preview hardening and its contract tests are deployed in Production. The Production global video gate is deliberately enabled for the internal-store physical-device acceptance window; the second store remains effectively disabled. The first physical iOS Safari attempt found a blocker where Stream was ready but DB readiness stayed `processing`, plus a video-card remove UI bug. Fixes are implemented locally and must be deployed before restarting physical iPhone Safari, physical Android Chrome, and the 72-hour retained Android review. See [[Review_Video_Canary_Runbook]], [[Review_Video_Physical_Device_Acceptance_2026-06]], and [[ADR_0031_Review_Media_V2_Provider_Agnostic_Video]].
