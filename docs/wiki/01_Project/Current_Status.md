...
## In Progress
- Video review production acceptance: the internal-store provider chain is active for controlled physical-device acceptance, while the second store remains effectively disabled. Quota-aware capability commit `84276d8b` is deployed in Production as `dpl_AjWAf29bnLSQWxA8ceQ8gdLBQQiz`. The internal quota was raised from `5` to `20` through dry-run-first exact-store operations; live capability returns `enabled=true` for the internal store and `merchant_disabled` for the second store. Physical Android Chrome and the 72-hour retained-review window remain pending. See [[Review_Video_Canary_Runbook]], [[Review_Video_Physical_Device_Acceptance_2026-06]], and [[ADR_0031_Review_Media_V2_Provider_Agnostic_Video]].
