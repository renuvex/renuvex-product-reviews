...
## In Progress
- Video review production acceptance: the internal-store provider chain is active for controlled physical-device acceptance, while the second store remains effectively disabled. After deploying the iOS readiness/removal fix, the physical iOS Safari critical path passed through upload, submit, admin approval, and storefront publication. The quota boundary then exposed a shopper UX gap. A local quota-aware capability implementation now checks live usage before every wizard open, degrades failures to photo-only, maps stable initiate errors to specific copy, and adds read-only admin usage status without changing merchant intent. It still requires verification, deploy, internal quota `20` dry-run/apply, Android Chrome acceptance, and the 72-hour retained-review window. See [[Review_Video_Canary_Runbook]], [[Review_Video_Physical_Device_Acceptance_2026-06]], and [[ADR_0031_Review_Media_V2_Provider_Agnostic_Video]].
