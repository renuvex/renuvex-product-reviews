...
## In Progress
- Video review production acceptance: the internal-store provider chain has produced two consumed quota units and completed prepare/publish/ingest-cleanup/video-cleanup jobs without retained media. Pending-admin preview hardening and its contract tests are deployed in Production. The Production global video gate is now deliberately enabled for the internal-store physical-device acceptance window; the second store remains effectively disabled. Physical iPhone Safari, physical Android Chrome, and the 72-hour retained Android review remain open. See [[Review_Video_Canary_Runbook]], [[Review_Video_Physical_Device_Acceptance_2026-06]], and [[ADR_0031_Review_Media_V2_Provider_Agnostic_Video]].
