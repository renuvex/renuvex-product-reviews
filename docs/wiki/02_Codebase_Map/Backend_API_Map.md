...
|---|---|---|
| GET `/api/admin/reviews?page&limit&status` | [route.ts](src/app/api/admin/reviews/route.ts) | Offset-paginated reviews for `merchantId`, ordered deterministically by `createdAt DESC, id DESC`. The bounded moderation projection includes the stored optional review title as well as rating, comment, product snapshot and action fields; exact pagination count remains a separate query. |
...
| GET `/api/admin/reviews/video-thumbnail?mediaId=` | [route.ts](src/app/api/admin/reviews/video-thumbnail/route.ts) | Tenant-scoped, short-lived Mux thumbnail URL for the admin review row. The route resolves only ready signed video belonging to the authenticated store and issues a 15-minute thumbnail-only JWT. Responses are `private, no-store`; playback credentials and provider identifiers are not returned. |
| GET `/api/admin/reviews/video-playback?mediaId=` | [route.ts](src/app/api/admin/reviews/video-playback/route.ts) | Short-lived signed Mux Player attributes (`playbackId`, `playbackToken`, `thumbnailToken`) for the full pending/admin video preview modal. The list-thumbnail path is separate and does not receive playback authority. Provider ids stay server-side. |
...
| GET `/api/public/ratings-by-slug?storeId&slugs=a,b,c` | [route.ts](src/app/api/public/ratings-by-slug/route.ts) | DOM-only fail-closed hint: resolve a slug only through exactly one fresh `active_verified` snapshot with no unknown/stale/conflict candidate. Freshness requires either a point-exact tuple matching the current installation or current coverage linked to a completed run and measured from its `startedAt`; tuple-less legacy timestamps do not qualify. The route then reads `ProductReviewSummary` by product id and never queries `Review.slug`. Backend and live Worker return `Cache-Control: no-store`; the 2026-08-09 `A0-EDGE` acceptance observed repeated `DYNAMIC/BYPASS` and no edge `HIT`. |
| GET `/api/public/settings?publicApiKey=<merchantId>` | [route.ts](src/app/api/public/settings/route.ts) | Pure cacheable widget config read (per widgetId) plus public runtime metadata. `runtime.placementPolicy` is the versioned auto-placement authority, `runtime.themeAdapterKey` is adapter selection, and `runtime.themeSyncDue` is only a lazy resync signal. The v1 cutover deliberately emits legacy `runtime.autoPlacementEnabled=false`; old runtimes fail closed while the current runtime reads `placementPolicy`. The route does not read auth tokens, call ikas, or schedule theme sync. Cloud name **not** in response — it is build-time injected into the widget bundle (see [[ADR_0008_Cloud_Name_Build_Time_Only]] and [[ADR_0038_Runtime_Attested_Storefront_Placement]]). |
...
## Change Log
- 2026-08-09: Closed the live slug no-store and manual product-reconciliation
  dispatch gates. Worker version `0bc1674d-331e-4953-a192-7f72c32d0fc4`
  returned repeated `no-store/DYNAMIC/BYPASS`; a `202` manual sync completed
  33/33 products through QStash and made the Production ready verifier pass.
...
- 2026-08-02: Admin review rows now resolve ready signed-Mux posters through a tenant-scoped thumbnail-only endpoint. Raw tokenless signed posters and provider identifiers are removed from the list response; the full playback endpoint remains isolated to the preview modal.
