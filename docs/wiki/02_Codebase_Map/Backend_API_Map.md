...
| GET `/api/admin/daily-maintenance` (Bearer CRON) | [route.ts](src/app/api/admin/daily-maintenance/route.ts) | Vercel cron: daily batch storefront theme verification plus pending upload cleanup and storefront script reconciliation. Legacy image metadata backfill is removed after the AWS-only image teardown source pass; route also supports lightweight sub-daily theme verification if the deploy plan supports it |
