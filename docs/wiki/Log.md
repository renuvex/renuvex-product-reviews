...
- Storefront review lightbox playback now has five merchant-editable `Video Oynatıcı` colors: play icon, play button background, button hover background, progress color, and progress track color. The keys are scoped to `reviewLightboxVideo*`, so admin moderation and future story/carousel players remain isolated.
