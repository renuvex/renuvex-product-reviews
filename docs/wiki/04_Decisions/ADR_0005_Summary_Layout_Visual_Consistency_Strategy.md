...
## Decision
1. **Vertical Spacing**: Standardize the vertical gap (`--renuvex-pr-col-gap`) between major summary blocks to **20px** for the Ozy theme. This provides a more premium, spacious feel.
2. **Button Synchronization**: Use `align-items: stretch` on action wrappers (`.renuvex-pr-summary-actions`, `.renuvex-pr-split-right`) and `height: auto` on buttons to ensure the Filter button exactly matches the height of the Write Review button, regardless of font scale.
