...
- Public variants: `reviews/<assetId>/<variant>.<format>`.
...
- The cleanup object family is `storeId + assetId`: private objects stay under the store-scoped private prefix, while public objects use the shorter `reviews/<assetId>/` prefix. The transitional legacy public prefix remains deleteable until the post-acceptance cleanup gate.
...
- `thumbnailUrl`: public WebP thumbnail URL for `thumb_320x427`.
...
- AWS image trust accepts only the approved delivery host and `reviews/<assetId>/<variant>.<format>` public key shape.
...
- Approved storefront images are public, immutable CloudFront variant URLs under `reviews/<assetId>/<variant>.<format>`.
...
- The review-image distribution needs at least these path behaviors:
  ...
  - `reviews/*`: unsigned public delivery; allowed methods `GET, HEAD`; origin is the same private S3 bucket through OAC.
  - `review-images/v1/public/*`: transitional unsigned legacy public delivery kept only until the post-acceptance cleanup gate removes old pre-public test paths.
...
- The scanned set comes from AWS inventory/metadata filtered primarily to `review-images/v1/private/stores/`, grouped into asset families. The transitional legacy public prefix may still be scanned while old test objects exist. The new public `reviews/<assetId>/` prefix does not contain `storeId`, so DB/manifest truth must be used when deleting public variants.
...
- Delete exact known keys first: private original, private variants, and public variants for the finite variant set.
- If the implementation needs to list, list only deterministic family prefixes for that asset: private `review-images/v1/private/stores/<storeId>/assets/<assetId>/`, public `reviews/<assetId>/`, and the transitional legacy public prefix. Never list or delete a whole store prefix or bucket prefix from a single family cleanup job.
...
- No lifecycle rule may expire `renuvex-state=committed` objects, the `reviews/` public prefix, or the transitional `review-images/v1/public/` prefix unless a separate product retention policy is approved.
...
CloudFront S3 read access must remain narrower than the whole review-image prefix. The bucket policy may allow the CloudFront distribution to read `reviews/*`, transitional `review-images/v1/public/*`, and private admin-preview variants under `review-images/v1/private/stores/*/assets/*/variants/*`; it must not allow CloudFront to read private originals such as `review-images/v1/private/stores/<storeId>/assets/<assetId>/original.<ext>`.
