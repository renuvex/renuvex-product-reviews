...
**Fix:** `-webkit-tap-highlight-color:transparent` was added to `HOST_RESET_CSS` in `core/shadow.js` rather than only patching the wizard with `BASE_RESET_CSS`. The property is **inherited**, so setting it on `:host` cascades to every element in every shadow surface (section, lightbox, wizard, and any future surface) regardless of class. `BASE_RESET_CSS` keeps its `renuvex-pr-` class-prefix scoping because it is also injected into `document.head` (light DOM, via `ensureBaseReset`) where it must not touch merchant elements; inside a shadow root that constraint does not exist, so the host-level reset is the correct altitude.

**Parity completion (same day):** `BASE_RESET_CSS` was then also added to the wizard shadow injection (`modal-shell.js` → `HOST_RESET_CSS + BASE_RESET_CSS + FWIZARD_CSS`), matching the section and lightbox. Removing the tap-highlight alone left wizard buttons with no press-state feedback on mobile (the blue flash had implicitly served that role); `BASE_RESET_CSS` restores the rest of the ADR_0011 contract — the deterministic `:active{opacity:0.85}` press dip (the intended replacement for the flash) plus `touch-action:manipulation` (no double-tap-zoom on rapid star taps) and `-webkit-touch-callout:none`. `user-select:none` in BASE_RESET is scoped to buttons / `[role]`, so wizard input/textarea text selection is unaffected. All three review shadow surfaces now carry the identical touch contract.
