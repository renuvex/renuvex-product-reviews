...
- 2026-05-30: Widget icons fully on Phosphor ([[Bug_Icon_Sprite_Inner_Dimension_Strip]]) — all chrome glyphs + filter `lines` + compact chevron via shared `icons/ui-icons.js`/`iconUseNode()`. No off-family widget glyph remains (admin `lucide-react` separate).
