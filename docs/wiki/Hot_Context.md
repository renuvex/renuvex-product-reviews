...
- 2026-06-23: Review Video playback moved from custom native `<video>` + direct `hls.js` orchestration to official Mux Player. Storefront media responses expose additive public `playbackId` for videos, the widget lazy-loads `@mux/mux-player` on lightbox open, Mux Data tracking/cookies stay disabled in this phase, and admin preview uses signed Mux Player attributes from `/api/admin/reviews/video-playback`.
