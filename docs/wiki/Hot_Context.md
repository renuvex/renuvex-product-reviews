...
- 2026-07-08: CloudFront standard logging v2 bucket/delivery stacks are created; final acceptance is pending a bucket stack update so policy/lifecycle target AWS's effective `AWSLogs/{account-id}/CloudFront/cloudfront/media/` prefix. See [[AWS_Setup_And_Access]].
