...
- 2026-05-30: Icon sprite fix + Phosphor wizard icons ([[Bug_Icon_Sprite_Inner_Dimension_Strip]]). `svgStringToSymbol` stripped `width`/`height` globally, erasing the image icon's `<rect>` frame; now root-`<svg>`-scoped. Wizard photo/plus icons swapped to Phosphor (matching badge/filter).
