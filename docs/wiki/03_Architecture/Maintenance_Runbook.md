...
| `source:media-job` + `action:reconcile_stream` | Stream webhook was delayed/missed or canonical status polling failed | The bounded self-healing schedule runs at 10/20/30/45/60/90/120/180/300/600 seconds and applies state through the same transition helper as the webhook. `readyToStream=true` plus provider `ready`, trusted HLS/poster URLs, and valid V1 media limits are authoritative; `pctComplete` is diagnostic only. After 10 minutes it records `stream_processing_delayed`; it does not delete or fail a still-processing video. Inspect Stream state, the 60-minute ingest job, and the 24-hour session expiry before manual mutation. |
