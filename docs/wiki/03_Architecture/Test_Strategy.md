...
Widget runtime smoke also pins review list-pagination sizing: the load-more button and numbered-pagination controls follow `Widget Boyutu` for desktop visible dimensions, while mobile separates the visible compact label box from the 44px hit target. The mobile numbered control must wrap without horizontal overflow, keep the tap target, and draw focus on the visible box rather than the invisible hit area.
