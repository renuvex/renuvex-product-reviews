...
- ADR 0038 production acceptance is not replaced by local placement tests. The first Product ID Worker canary proved PDP/category/homepage/slider/infinite-scroll but found an actual Ozy quick-view generation-rollover availability bug. Its local regression tests do not close that gate. Backend and Worker runtime deployments are separate approvals; live Ozy PDP/listing/homepage/search/slider/modal Product ID attributes, edge payload/no-store behavior, manifest hash, request cardinality, observer behavior, one natural lifecycle reconciliation, and the repeated canary must be recorded after approved stages.
