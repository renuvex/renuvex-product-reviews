...
| Storefront placement capability | `pnpm test:widget-placement` | Focused production-runtime browser contract for ADR 0038: provider/runtime policy selection, strict Ozy PDP/listing/modal proofs, delayed DOM, context-epoch cancellation, recycled cards, modal identity, malformed/legacy policy fail-closed behavior, v2 settings-cache cutover, and the retained pre-policy runtime artifact consuming legacy `autoPlacementEnabled: false`. |
...
- Browser safety: ambiguous, partial, stale, recycled, unsupported, malformed-policy, and old-runtime/new-backend cases produce no rating request and no automatic DOM mutation.
- Browser availability: exact Ozy PDP, category/listing, no-media cards, delayed DOM, and trusted quick-view modal paths render the expected badge.
- Cache compatibility: the v2 key is independent, valid v1 policy removes the legacy key once, and stale v2 settings disable automatic placement while preserving unrelated settings behavior.
...
- ADR 0038 production acceptance is not replaced by local placement tests. Backend policy deployment and Worker runtime deployment are separate approvals; live Ozy PDP/listing/homepage/slider/modal availability, edge payload, manifest hash, request cardinality, and observer behavior must be recorded after each approved stage.
