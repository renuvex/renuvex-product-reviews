...
Widget runtime smoke also pins review list-pagination sizing: the load-more button and numbered-pagination controls follow `Widget Boyutu` for font and physical dimensions, while the mobile numbered control wraps without horizontal overflow and keeps a 44px minimum hit target.
