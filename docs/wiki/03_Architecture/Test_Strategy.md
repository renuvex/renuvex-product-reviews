...
- ADR 0038 production acceptance is not replaced by local placement tests. PR #40 and the separately approved Worker rollout completed, and Canary 1 passed fresh desktop plus measured `412x915` PDP/listing/homepage/search/slider/modal Product ID checks, beyond-TTL quick-view lifetime, edge payload/no-store behavior, runtime hashes, and bulk request cardinality. The `49/49` placement and five-browser `20/20` regressions remain CI evidence rather than substitutes for that live canary. One natural lifecycle reconciliation, the repeated Canary 2, and Sentry alert delivery still require independent production evidence.
