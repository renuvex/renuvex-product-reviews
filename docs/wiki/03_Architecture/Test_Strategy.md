...
| Admin preview/settings | `pnpm test:admin-preview` | Preview `postMessage` update path, layout/icon/color/toggle effects, nested-iframe wheel recovery after a modal pointer-lock cycle, and static `widgetDefs.ts` option/showWhen alignment with widget registries. |
