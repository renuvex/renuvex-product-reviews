...
source_files:
  ...
  - "scripts/benchmark-product-lifecycle-scale.ts"
  ...
  - "tests/unit/product-lifecycle-verifier.test.ts"
  - "tests/unit/product-snapshots.test.ts"
  - "tests/unit/product-reconciliation.test.ts"
  - "tests/unit/product-reconciliation-route.test.ts"
  - "tests/unit/product-reconciliation-sweep.test.ts"
  - "tests/unit/product-reconciliation-sweep-route.test.ts"
  - "tests/unit/product-reconciliation-dispatcher.test.ts"
  - "tests/unit/product-lifecycle-retention.test.ts"
...
| Widget network/chunk smoke | `pnpm test:widget-smoke` | Built `public/widget.js` and content-hashed runtime chunks; validates API fan-out, lazy chunk boundaries, badge/review/structured-data combinations, unsupported theme behavior, strict listing gating, SPA product retirement without false health telemetry, and local transfer evidence without byte-budget gating. `tests/unit/widget-health.test.ts` separately pins that lifecycle-retired probes stay quiet while relevant missing nodes still report. |
| Storefront placement capability | `pnpm test:widget-placement` | Focused production-runtime browser contract for ADR 0038: provider/runtime policy selection, strict Ozy PDP/listing/modal proofs, event-generation identity conflicts, DOM-only Product ID promotion, delayed responses, recycled cards and modals, malformed/legacy API and policy fail-closed behavior, v2 rating-cache rejection, and retained-runtime compatibility. |
| Critical Product ID placement | `pnpm test:widget-placement:critical` / `pnpm test:widget-placement:cross-browser` | Four compact acceptance paths verify PDP slot/inner Product ID, DOM-only listing promotion, event-ID listing plus quick-view continuity, and Product ID-less legacy response fail-closed behavior. The PR gate runs Chromium desktop, Pixel Android, and iPhone WebKit; the full command and daily workflow also run Firefox desktop and desktop WebKit. |
...
| Cross-browser storefront | `pnpm test:widget-media` plus Product ID placement scripts | Local fast paths run Chromium desktop. CI runs PR media coverage and the critical Product ID placement suite as isolated matrix jobs for Chromium desktop, Pixel Android emulation, and iPhone WebKit emulation. The scheduled workflow adds Firefox desktop and desktop WebKit. Media coverage pins poster-first rendering, Mux playback/upload, browser-back, and cleanup; placement coverage pins canonical Product ID ownership and fail-closed legacy compatibility. |
...
| Product lifecycle evidence | Focused lifecycle Vitest files plus `pnpm test:integration:review-email` | Pure tests cover two daily exact-empty slots separated by at least 24 hours, explicit deletion, sticky conflict, tuple-aware exact freshness, linked-run coverage freshness, safe slug resolution, malformed/duplicate provider pages, delayed retry, truthful publish failure, opaque run/sweep continuations, terminal observation cleanup, bounded 42-day retention, and reinstall stale fencing. Disposable PostgreSQL proves the all-or-none exact-provenance constraint, stale-fence rejection before snapshot writes, coverage preservation of exact provenance/timestamp, changed-only persistence, one global nonterminal sweep, tombstone/conflict persistence, same-slug/new-id isolation, and transactional stale closure/observation deletion. PG16/17 jobs run both `--expect=expanded` and the readiness query on an empty disposable database so the full SQL contract executes on both engines; live `--expect=ready` with current installation data and natural continuity remains a separate post-deploy gate. |
...
Placement acceptance is split deliberately:

- Source correctness: static boundaries prove production PDP/listing/modal paths cannot import the removed broad discovery helpers and preview stays isolated.
- Browser safety: ambiguous, partial, stale, recycled, unsupported, malformed-policy/identity/rating, conflict, 429/5xx/network, and new-runtime/old-API cases produce no visible badge.
- Browser availability: exact Ozy PDP, category/listing, no-media and duplicate-product cards, delayed DOM/response, mixed event/discovery batches, and trusted quick-view modal paths render Product ID-stamped badges. Quick-view also covers an Ozy-style same-route listing generation that re-attests the exact clicked link with the same sealed Product ID, a bound modal surviving the former discovery TTL, and non-link interaction inside that exact modal.
- Race safety: event-generation changes, A-to-B-to-A SPA navigation, href/title/mount mutation, card/modal recycling, multiple/hidden/closed modals, repeated mutation, and partial batch failure are revalidated before mutation. A different Product ID after modal click must update the listing proof while leaving the modal without a badge; an unsealed ID cannot cross a generation boundary. An unbound context must expire after the discovery TTL, while disconnecting the attested source must retire an already bound modal badge.
- Cache compatibility: settings v2 remains independent; rating cache v2 is ignored, rating cache v3 is Product ID-only with per-entry TTL, old runtime ignores the additive API Product ID, and unresolved discovery is proof-scoped rather than persisted.
- Browser matrix: PR requires Chromium desktop, Pixel Android, and iPhone WebKit for the four critical Product ID paths; daily coverage adds Firefox desktop and desktop WebKit.
- Deployment acceptance: origin settings, Worker edge settings and `HIT/MISS`, manifest entry, immutable runtime hash, and fresh-browser behavior are separate evidence. CI does not prove any of those artifacts are live.

An already-running legacy JavaScript context cannot be revoked remotely. Fresh page load/reload plus a successful settings read is the rollout boundary; network-failure stale legacy cache and tabs that never refetch remain bounded migration risks until those contexts end.
...
- ADR 0038 production acceptance is not replaced by local placement tests. PR #39 fixed and deployed the first Ozy quick-view generation-rollover failure; the next canary proved the correct badge mounted but exposed discovery TTL revoking the same bound modal at about `14.48 s`. The `49/49` placement and five-browser `20/20` lifetime regressions do not close that live gate. Backend and Worker runtime deployments are separate approvals; live Ozy PDP/listing/homepage/search/slider/modal Product ID attributes, beyond-TTL modal lifetime, edge payload/no-store behavior, manifest hash, request cardinality, observer behavior, one natural lifecycle reconciliation, and the repeated canary must be recorded after approved stages.
...
- Sentry production health/alert checks are not part of CI. The Product ID closeout requires a first-event `identity-conflict` alert plus an aggregate 10 events / 5 minutes alert for the other three identity/placement health types; creating those external rules remains an explicit mutation gate.
