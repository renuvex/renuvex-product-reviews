...
- Browser availability: exact Ozy PDP, category/listing, no-media and duplicate-product cards, delayed DOM/response, mixed event/discovery batches, and trusted quick-view modal paths render Product ID-stamped badges.
- Race safety: event-generation changes, A-to-B-to-A SPA navigation, href/title/mount mutation, card/modal recycling, multiple/hidden/closed modals, repeated mutation, and partial batch failure are revalidated before mutation.
