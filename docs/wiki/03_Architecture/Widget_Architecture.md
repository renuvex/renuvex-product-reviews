...
15. Resource identity changes must clear stale visible content before async reads finish. When a PDP surface receives a different canonical `productId`, it should keep the owned slot/reservation shell but remove the previously rendered product's content immediately, reset surface state for the new identity, and ignore late responses from older identities. Duplicate events for the same identity must stay idempotent and must not clear already-correct content.
...
- PDP product transitions are identity-aware. If a previously rendered review widget belongs to product A and ikas emits `PRODUCT_VIEW` for product B, `bootstrap.js` clears the shadow content into the existing quiet `#renuvex-reviews` shell before settings or reviews fetches complete. This prevents product A reviews from staying visible on product B while preserving the layout reservation. Same-product duplicate `PRODUCT_VIEW` events are left intact.
...
## Change Log
- 2026-07-02: Added identity-aware PDP review transition reset. On SPA product changes, a rendered review widget for the previous `productId` is cleared into the existing reserved shell before new settings/reviews finish loading, while duplicate same-product events remain idempotent and late responses stay guarded by the existing bootstrap token.
