...
3. Widget calls `READ_API_BASE /api/public/settings`, evaluates the versioned `runtime.placementPolicy`, and then calls the surface-specific public read. In the Worker delivery target, settings is stored in the PoP-local Cache API for 60 seconds while the browser-facing response remains `max-age=0, must-revalidate`; the Worker Cache API does not provide backend `stale-while-revalidate` or `stale-if-error` semantics. If settings returns `runtime.themeSyncDue`, the widget sends a non-blocking `POST API_BASE /api/public/storefront-theme/lazy-sync`; this signal does not authorize placement. `ASSET_BASE` is `https://widget.renuvex.app`, `API_BASE` is `https://app.renuvex.app`, and `READ_API_BASE` is `https://widget.renuvex.app` for allowlisted public settings/ratings/reviews reads. See [[ADR_0038_Runtime_Attested_Storefront_Placement]].
