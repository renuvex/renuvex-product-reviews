...
export async function POST(request: Request) {
  try {
    ...
    if (!fileName || fileName.length > 255 || !contentType || !bytes || !checksumSha256) {
      return withAnonymousPublicCors(NextResponse.json({ error: 'Invalid image upload request.' }, { status: 400 }));
    }
    ...
    return withAnonymousPublicCors(NextResponse.json({ error: 'Upload signature could not be created.' }, { status: 500 }));
  }
}
