...
async function visibleCountInOverlay(page: Page, overlaySelector: string, selector: string) {
  return page.evaluate(({ overlaySelector, selector }) => {
    const root = Array.from(document.querySelectorAll('[data-renuvex-shadow-overlay]'))
      .map((host) => (host as HTMLElement & { shadowRoot: ShadowRoot | null }).shadowRoot)
      .filter((candidate): candidate is ShadowRoot => !!candidate)
...
test('video upload wizard posts a ready video token without photo media', async ({ page }) => {
  ...
  expect(await countInOverlay(page, '.renuvex-pr-fwizard-overlay', '.renuvex-pr-fwizard-video-thumb .renuvex-pr-fwizard-video-remove')).toBe(1);
  expect(await countInOverlay(page, '.renuvex-pr-fwizard-overlay', '.renuvex-pr-fwizard-video-thumb .renuvex-pr-fwizard-photo-add')).toBe(0);
...
test('video retry keeps the selected file after repeated stalled uploads', async ({ page }) => {
  ...
  const videoToken = 'video-token-stalled-manual-retry-abcdefghijklmnopqrstuvwxyz1234567890';
  let initiateCalls = 0;
  let completeCalls = 0;
  let muxPutCalls = 0;
  let statusReady = false;
  ...
  await page.route(`${WIDGET_ORIGIN}/api/public/upload/video/initiate**`, async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        data: {
          token: videoToken,
          uploadUrl: 'https://mux-upload.test/stalled-manual-retry',
          chunkSize: 8192,
          chunkAttempts: 5,
          expiresAt: '2099-01-01T00:00:00.000Z',
        },
  ...
  await page.route('https://mux-upload.test/stalled-manual-retry**', async (route) => {
    muxPutCalls += 1;
    if (muxPutCalls <= 2) {
      await new Promise((resolve) => setTimeout(resolve, 1500));
      try {
