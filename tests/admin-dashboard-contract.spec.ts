...
test('dirty editor navigation uses one modal and discard leaves without saving', async ({ page }) => {
  const log = await setupAdminDashboardRoutes(page);
  ...
  await expect(dashboard.getByText('Widgetlar', { exact: true }).last()).toBeVisible();
...
test('save-and-exit clears the native unload boundary before shell navigation', async ({ page }) => {
  const log = await setupAdminDashboardRoutes(page);
  const dashboard = await openAdminHarness(page, 'success', '/dashboard/widgets/reviews');
